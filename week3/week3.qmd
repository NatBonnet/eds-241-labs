...
weekly_la_phx <- did_data_az_ca %>% 
  ...
  group_by(week, city) %>% 
  summarise(mean_no2 = mean(daily_max_1_hour_no2_concentration, na.rm = TRUE), 
            .groups = "drop")
...
ggplot(weekly_la_phx, aes(x = week, y = mean_no2, color=city))+
  ...
  # Add line to show lockdown
  geom_vline(xintercept = as.Date("2020-03-19"), 
             linetype = "dashed", 
             color = "grey", 
             size = 1) +
  # Add annotation for start of covid lockdown
  annotate("label", x = as.Date("2020-03-20"), 
           y = 35, 
           label = "California Lockdown \n starts", 
           size = 3) +
  labs(title = "Weekly NO2 Trends in 2020: LA vs. PHX", 
       x = "Date", 
       y = "Weekly average max NO2 (ppm)", 
       color = "City") +
  theme_minimal() +
...
**This violates the parallel trends assumption of DiD**
...
y_trt_post <- did_data_nm_tx %>% 
  filter(treated == 1, is_post == 1, year == 2020, month(date_obj) %in% c(2, 3, 4))%>% 
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% 
  pull()

y_trt_pre <- did_data_nm_tx %>% 
  filter(treated == 1, is_post == 0, year == 2020, month(date_obj) %in% c(2, 3, 4))%>% 
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% 
  pull()

y_con_pre <- did_data_nm_tx %>% 
  filter(treated == 0, is_post == 0, year == 2020, month(date_obj) %in% c(2, 3, 4))%>% 
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% 
  pull()

y_con_post <- did_data_nm_tx %>% 
  filter(treated == 0, is_post == 1, year == 2020, month(date_obj) %in% c(2, 3, 4))%>% 
  summarise(mean(daily_max_1_hour_no2_concentration)) %>% 
  pull()
...
```{r}
tx_nm_did <- lm(daily_max_1_hour_no2_concentration~treated * is_post, data = filter(did_data_nm_tx, year == 2020, month(date_obj) %in% c(2, 3, 4)))

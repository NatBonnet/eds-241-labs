...
*Response:* reef exposure (Exposed) and pollution risk (low) look the most different between MPA and non-MPA
...
*Response:* MPA groups already have some covariates that will make it look as if the MPA has a bigger effect- for exposure, more wave action might deliver more nutrients to primary producers
...
*Response:* How much does MPA treatment increase fish biomass compared to similar sites?
...
*Response:* treated units was 108, control was 37

**Q6.** Find the covariate with the largest standardized mean difference (SMD) *after matching*. Which covariate is it?

*Response:* After matching the standard mean difference of distance to deep water got larger.
...
```{r}
reg_ecological <- lm(biomass_ecological~treated_mpa, data = matched_data, 
                    weights = weights)
...
```{r}
reg_fisheries <- lm(biomass_fisheries~treated_mpa, data = matched_data, 
                    weights = weights)
...
*Response:* Looking at the fisheries regression, there is an increase in average biomass in MPA treatment sites compared to non-MPA treatment of 107 kg/ha. 

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* There may still be unobserved covariates not included in the model. These matching experiments do not account for unobserved selection bias.
...
*Response:* vpd increases on fixed effect model, this means that plot and year were confounding the weight of  vapor pressure deficit in the first model without fixed effects.
...
*Response:* Year helps include factors such as drought, rain events, etc., so including it will help reduce the weight in treatment that should really be attributed to outside effects.
...
*Response:* Conditions will vary depending on plot so the effect of treatment will not necessarily be the same across plots- this might depend on geographic factors. 

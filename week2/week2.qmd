...
```{r}
m1_ols <- lm(waste_piles~treat, 
             data = post_treat_subset)

summ(m1_ols, model.fit = FALSE)
```
intercept(77.91): without treatment (control), we expect the number of trashpiles to be about 78 on average
treat(-24.77): treatment reduced average number of trash piles by about 25 on average 
...
```
intercept(78.26): We expect that the average waste pile count for untreated neighborhoods is 79. 
treat(0.68): We would expect a 32% rduction in waste pile counts
...
exp(-0.42)-1
```
Intercept: The average number of trash piles in non treated areas is 74.
treat: We expect an average 34% reduction in waste piles in treated areas.
...
export_summs(m1_ols, m2_poisson, m3_log, 
             model.names = c("OLS", "Poisson GLM", "Log(outcome) OLS"), statistics = "none" )
```
...
```{r}
m1_change = (-24.77/77.91)*100 # Change in OLS
m2_change = (exp(-0.38)-1) *100 # CHange in Poisson
m3_change = (exp(-0.42)-1)*100 #Change in log-ols
```
...
The treatment group already had a lower average- this means we might not be able to attribute all of the treatment effect to the treatment itself- need to control for these preexisting differences
...
```{r}
m4_control <- lm(log(waste_piles)~treat + log(pre_count_avg), 
                 data = post_treat_subset)
...
exp(0.85)
```
intercept: when both treat and precount average is at zero- in the control neighborhood we would expect 2 trash piles
treat: when controlling for pretreatment piles, the treatment would be expected to reduce 
log(precount): wiht every 1% increase in pre count waste pile count average, there is a 0.85% increase in post treatment waste count piles
Let's compare our two log OLS Regression Models. 

```{r}
export_summs(m3_log, m4_control, 
             model.names = c("Log OLS Regression", "Log OLS Regression with a control"), statistics = "none")
```

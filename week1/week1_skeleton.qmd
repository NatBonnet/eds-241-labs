...
balance_summary <- class_data %>% 
  gtsummary::tbl_summary(
    # select how to group columns
    by = random_groups, 
    # Variables to include
    include = c(height, pets, dominant_hand, fav_number), 
    # What calculation are we doing
...
```{r}
tbl_2way <- lobsters %>% 
  # Calculate the total catch per unit effort for year and region
  group_by(year, region) %>% 
  summarise(
    total_cpue = sum(cpue, na.rm = TRUE), 
    .groups = "drop" # Same as ungrouping
  ) %>% 
  # Pivot to creae a column for each region
  pivot_wider(
    names_from = region, 
    values_from = total_cpue
  ) %>% 
  # Arrange by year so everything is in order
  arrange(year) %>% 
...
tbl_2way %>%
  gt(rowname_col = "year") %>%
  tab_header(title = "European Lobster Catch by Region and Year", 
...
```{r}
tbl_3way <- lobsters %>% 
  # Calculate total CPUE for each year, region, and treatment group
  group_by(year, region, treat) %>% 
  summarise(total_cpue = sum(cpue, na.rm = TRUE), 
            .groups = "drop") %>% 
  # Pivot to create col for each treatment group within each region
...
tbl_3way %>% 
  gt(rowname_col = "year")%>%
  tab_header(title = "European Lobster Catch", 
             subtitle = "Total Catch Per Unit Effort (CPUE) by Year, Region, and Treatment Group") %>%
  tab_spanner(
    label = "Bolærne",
...
table_w_plots <- lobsters %>% 
  group_by(year) %>% 
  summarise(total_cpue = sum(cpue, na.rm = TRUE), 
            dist_cpue = list(cpue), # Store all indiviudal cpue vlas as a list for each year
            .groups = "drop") %>% 
  arrange(year) %>% 
  # Create gt table
  gt() %>% 
  tab_header(title = "European Lobster Catch Totals and Distribution (2006-2010)", 
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "egret"); beep(4)  
...
3. Discuss the matched-pairs design of the study. How were neighborhoods paired? Create a diagram if helpful!
4. Was the social competition strategy effective?
